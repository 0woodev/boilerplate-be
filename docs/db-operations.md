...
같은 key를 다른 타입/구조로 사용하는 경우. 예: `count: int` → `count: dict`.

```
PR #1 — 신규 필드 추가
  - 기존 count 유지 + count_v2: dict 신규
  - 쓰기 핸들러: 둘 다 채움
  - 읽기 핸들러: count_v2 우선, 없으면 count fallback

PR #2 — Backfill
  - 일회성 스크립트로 모든 아이템 count → count_v2 변환
  - 진행률 로그

PR #3 — 코드 정리
  - 읽기에서 count fallback 제거
  - 쓰기에서 count 채우기 제거

PR #4 (옵션) — 데이터 정리
  - cleanup 스크립트로 모든 아이템에서 count 필드 제거
```
...
예: `PK=ITEM_ID@xxx` → `PK=WS@yyy#ITEM_ID@xxx` (멀티테넌트 도입).
